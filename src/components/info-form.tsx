...
const NovusOnboardingCompact = () => {
    ...
    return (
        <div className="relative min-h-screen w-full bg-[#FFFBF8] overflow-hidden flex items-center justify-center font-sans text-slate-900 selection:bg-orange-100">
            ...
            <div className="relative z-10 w-full max-w-lg px-4">
                <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-[0_20px_40px_-15px_rgba(255,145,100,0.1)] border border-white/60 p-8 md:p-10 overflow-hidden">
                    ...
                    <div className={`transition-all duration-300 ease-in-out transform ${isAnimating ? '-translate-x-4 opacity-0' : 'translate-x-0 opacity-100'}`}>
                        ...
                        {/* Questions (Smaller Text) */}
                        <h1 className="text-3xl md:text-4xl font-bold mb-2 text-slate-900 tracking-tight">
                        ...
                        <div className="relative group">
                            <input
                                ...
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-lg text-slate-800 placeholder:text-slate-300 focus:outline-none focus:border-orange-400 focus:bg-white transition-all shadow-sm"
                            />

                            {/* Action Button */}
                            <button
                                onClick={handleNext}
                                className={`absolute right-2 top-2 bottom-2 aspect-square rounded-lg flex items-center justify-center transition-all duration-300 ${formData[questions[step].key]
                                    ? 'bg-slate-900 text-white hover:bg-orange-500 shadow-md cursor-pointer'
                                    : 'bg-slate-200 text-slate-400 cursor-not-allowed'
                                    }`}
                            >
                                {step === questions.length - 1 ? <Sparkles size={16} /> : <ArrowRight size={16} />}
                            </button>
                        </div>
